...
namespace ProjectFrameworkAPITest
{
    public partial class ProjectFrameworkForm : Form
    {
        ...
        private async void ButtonGet2_ClickAsync(object sender, EventArgs e)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri(textBoxURL.Text);
                ...
                HttpResponseMessage response = await client.GetAsync("api/Contact/GetContactInfo");
                string ResponseData = "";
                if (response.IsSuccessStatusCode)
                {
                    ResponseData = await response.Content.ReadAsStringAsync();

                    AppContact Contact = JsonConvert.DeserializeObject<AppContact>(ResponseData);
                    textBoxAddress.Text = Contact.Address;
                    textBoxSiteURL.Text = Contact.Site_URL;
                    textBoxContact.Text = Contact.Contact_No;
        ...
        private async void ButtonSet1_ClickAsync(object sender, EventArgs e)
        {
            AppContact Contact = new AppContact();
            Contact.Address = textBoxAddress.Text;
            Contact.Site_URL= textBoxSiteURL.Text;
            Contact.Contact_No = textBoxContact.Text;
